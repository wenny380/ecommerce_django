...
def signUpView(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            username = form.cleaned_data.get('username') #“cleaning” the input – normalizing it to a consistent format.
...
def loginView(request):
    if request.method == 'POST':
        form = AuthenticationForm(data=request.POST) #get the data from the form
        if form.is_valid():
            username = request.POST['username']
            password = request.POST['password']
            user = authenticate(username=username, password=password)
            if user is not None: # if the user is found redirect him to the home page
                login(request, user)
